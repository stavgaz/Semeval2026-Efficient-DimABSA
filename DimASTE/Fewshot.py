...
def load_prompt(lang, domain, is_train=True):
    """
    Load the few-shot prompt template for (lang, domain) from fewshot_prompts2.jsonl.

    NOTE: despite .jsonl extension, json.load() expects ONE JSON object in the file.
    """
    with open("./fewshot_prompts2.jsonl", "r") as f:
        prompts = json.load(f)
...
def create_prediction_prompt(
    ...
    The prompt includes a short, language-specific instruction to output only JSON.
    """
    instruction = load_prompt(language, domain, is_train=False)
    text = data_sample["Text"].replace("` ` ", "")

    # Initialize so we never return an undefined variable
    prompt = None
...
def get_original_span(full_text: str, predicted_span: str) -> str:
    """Map predicted span back to an exact substring in the original review text."""
...
def main():
    ...
    """
    parser = argparse.ArgumentParser(description="Few-shot inference (Qwen) for DimABSA subtask_2.")
