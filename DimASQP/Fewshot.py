...
# ============================================================
# Prompt template loading (few-shot)
# ============================================================

def load_prompt(lang, domain, categories, is_train=True):
    """
    Load the few-shot prompt template for (lang, domain) and insert allowed categories.
    fewshot_prompts3.jsonl must be a single JSON object (json.load).
    """
    with open("./fewshot_prompts3.jsonl", "r") as f:
        prompts = json.load(f)
...
# ============================================================
# Build inference prompt (Qwen chat format)
# ============================================================
...
def create_prediction_prompt(
    ...
    """
    Build a Qwen chat prompt that asks for JSON output.
    Includes a short “Output:” instruction in the user message for clarity.
    """
    ...
    # IMPORTANT: initialize to avoid UnboundLocalError if branches don't hit
    prompt = None
...
Тик БЕР ГЕНӘ JSON-массив чыгарыгыз:
<|im_end|>
<|im_start|>assistant
"""
        elif language == "rus":
...
def convert_prediction_data(
    ...
    """
    Convert raw dev/test samples to prompt JSONL:
      - ID
      - prompt_text
      - raw_text
    """
...
def main():
    parser = argparse.ArgumentParser(description="Few-shot inference (Qwen) for DimABSA.")
